...
- (void)layoutSubviews
{
    ...
    if (!isLayedOut)
    {
        ...
        {.origin = CGPointZero, .size = self.layer.frame.size};
        ...
        [letters enumerateObjectsUsingBlock:^(NSString *letter, NSUInteger idx, BOOL *stop)
        {
            ...
            CATextLayer *ctl = [self textLayerWithSize:fontSize string:letter andFrame:CGRectMake(0, originY, self.frame.size.width, letterHeight)];
...
- (CATextLayer*)textLayerWithSize:(CGFloat)size string:(NSString*)string andFrame:(CGRect)frame
{
    ...
    [tl setForegroundColor:RGB(132, 112, 225, 1).CGColor];
