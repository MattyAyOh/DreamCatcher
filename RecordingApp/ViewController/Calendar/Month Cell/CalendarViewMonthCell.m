...
@interface CalendarViewMonthCell () <UICollectionViewDataSource, UICollectionViewDelegate>
{
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        MonthCollectionViewFlowLayout* monthFlowLayout = [[MonthCollectionViewFlowLayout alloc] initWithCollectionViewSize:frame.size andHeaderHeight:[CalendarHeaderView height]];
        ...
        [self.collectionView registerClass:[CalendarViewDayCell class] forCellWithReuseIdentifier:NSStringFromClass([CalendarViewDayCell class])];
        
        [self.collectionView registerClass:[CalendarHeaderView class] forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:NSStringFromClass([CalendarHeaderView class])];
    }
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        CalendarHeaderView* headerView = (CalendarHeaderView*)[collectionView dequeueReusableSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:NSStringFromClass([CalendarHeaderView class]) forIndexPath:indexPath];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    // take the date back to the first day of the month
    
    offsetComponents.day -= _firstWeekdayOfMonthIndex;
    // make room for the days of the previous month (ex. If the first day of the month is a Tue then we need 2 slots)
    
    offsetComponents.day += indexPath.item;
    // add the day offset corresponding to the cell about to be drawn
    
    
    NSDate* dayCellDate = [_calendar dateByAddingComponents:offsetComponents toDate:_displayMonthDate options:0];
    
    NSCalendarUnit calendarUnit = NSCalendarUnitYear | NSCalendarUnitMonth | NSCalendarUnitDay;
    
    NSDateComponents *dayDateComponens = [_calendar components:calendarUnit fromDate:dayCellDate];
    ...
    {
        NSDateComponents *selectedDateComponens = [_calendar components:calendarUnit fromDate:self.dateSelected];
    ...
    shouldDisplayAsActive &= indexPath.item >= _firstDayActive && indexPath.item <= _lastDayActive;
    // is between the start and the end date
    
    shouldDisplayAsActive &= indexPath.item >= _firstWeekdayOfMonthIndex;
    // is at or after the first weekday of the month
...
- (void) collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CalendarViewDayCell* cell = (CalendarViewDayCell*)[collectionView cellForItemAtIndexPath:indexPath];
...
-(void)setDisplayMonthDate:(NSDate *)monthDate
{
    ...
    _monthComponents = [self.calendar components:calendarUnit fromDate:_displayMonthDate];
    ...
    _firstWeekdayOfMonthIndex = [self.calendar component:NSCalendarUnitWeekday fromDate:dateFromComponents] - 1;
    // _firstDayOfMonthIndex should be zero indexed
    
    _numberOfDaysInMonth = [self.calendar rangeOfUnit:NSCalendarUnitDay inUnit:NSCalendarUnitMonth forDate:_displayMonthDate].length;
    
    _todayDateComponents = [self.calendar components:calendarUnit fromDate:[NSDate date]];
...
{
    return [NSString stringWithFormat:@"<CalendarViewMonthCell %p (display:%@, date:%@)>", self, self.displayMonthDate, self.dateSelected];
}
@end
