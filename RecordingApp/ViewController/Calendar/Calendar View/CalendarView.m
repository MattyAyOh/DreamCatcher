...
@property (nonatomic, readonly) CalendarViewMonthCell* currentMonthCell;
...
-(void)setup
{
    ...
    self.collectionView = [[UICollectionView alloc] initWithFrame:sFrame collectionViewLayout:flowLayout];
    ...
    [self.collectionView registerClass:[CalendarViewMonthCell class] forCellWithReuseIdentifier:NSStringFromClass([CalendarViewMonthCell class])];
...
- (NSInteger)numberOfSectionsInCollectionView:(UICollectionView *)collectionView
{
    if(self.dataSource)
    {
        ...
        NSDateComponents *components = [_calendar components:(NSCalendarUnitYear | NSCalendarUnitMonth |NSCalendarUnitDay)fromDate:_startDateCache];
...
#pragma GCC diagnostic ignored "-Wundeclared-selector"
        ...
        else
        {
            ...
            _endDateCache = [_calendar dateByAddingComponents:offsetComponents toDate:_startDateCache options:0];
            
        }
        
        if(!self.showsWholeMonthsOnStartAndEnd)
            
            // if we select NOT to show all the days of the first and last month (default) we must calculate the difference and cache it.
        {
            _firstDayActive = [_calendar component:NSCalendarUnitDay fromDate:_startDateCache];
            
            _lastDayActive = [_calendar component:NSCalendarUnitDay fromDate:_endDateCache];
        }
        else
        {
            _firstDayActive = 0;
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section
{
    ...
    if(self.delegate)
    {
        [self.delegate calendarController:self didScrollToMonth:self.dataSource.startDate];
    }
    ...
    _numberOfItemsInSectionCache = [_calendar components:NSCalendarUnitMonth fromDate:_startDateCache toDate:_endDateCache options:0].month + 1;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CalendarViewMonthCell *monthCell = (CalendarViewMonthCell*)[collectionView dequeueReusableCellWithReuseIdentifier: NSStringFromClass([CalendarViewMonthCell class])forIndexPath: indexPath];
    ...
    monthCell.displayMonthDate = [_calendar dateByAddingComponents:offsetComponents toDate:_startOfMonthCache options:0];
    
    
    monthCell.firstDayActive = indexPath.item == 0 ? _firstDayActive : 0;
    // if it is the first cell then it might have an offset to its start
    
    monthCell.lastDayActive = indexPath.item == (_numberOfItemsInSectionCache - 1) ? _lastDayActive : 31;
    
    // if it is the last cell it might not finish at the end, otheriwe set it to the largest number possible
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(collectionView != self.collectionView) // * This is the callback from the collection view inside each month cell, it refers to day cells
    {
        ...
        {
            [self.delegate calendarController:self didSelectDay:_dateSelected];
        }
    }
...
-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    if(self.delegate && [self.delegate respondsToSelector:@selector(calendarController:didScrollToMonth:)])
    {
        ...
        [self.delegate calendarController:self didScrollToMonth:self.currentMonthCell.displayMonthDate];
    }
...
- (void) setDateSelected:(NSDate *)dateSelected animated:(BOOL)animated
{
    ...
    NSInteger month = [_calendar components:NSCalendarUnitMonth fromDate:_startDateCache toDate:dateSelected options:1].month;
...
-(CalendarViewMonthCell*)currentMonthCell
{
    ...
    return (CalendarViewMonthCell*)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:pageNumberRoundedDown inSection:0]];
...
- (void)setMonthDisplayed:(NSDate *)monthDisplayed animated:(BOOL)animated
{
    ...
    NSDateComponents* differenceComponents = [_calendar components:NSCalendarUnitMonth fromDate:_startDateCache toDate:monthDisplayed options:0];
...
-(void) setDataSource:(id<CalendarDataSource>)dataSource
{
...
- (void) setShowsEvents:(BOOL)showsEvents
{
    ...
    if(showsEvents && !_showsEvents &&  _startDateCache)
        // if we set to YES and the view has fetched the dates from the delegates
    {
        [self loadEventsInCalendar];
...
- (void) loadEventsInCalendar
{
    ...
    void(^FetchEventsBlock)(void) = ^{
        ...
        NSPredicate *predicate = [_store predicateForEventsWithStartDate:_startDateCache endDate:_endDateCache calendars:nil];
        ...
        __weak CalendarView* wself = self;
